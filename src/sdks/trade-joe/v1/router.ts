...
export class JoeRouter extends EvmContract {
  ...
  async createPair(tokenX: string, tokenY: string): Promise<string> {
    const factory = new JoeFactory(await this.getFactory(), this.signer)
    return await factory.createPair(tokenX, tokenY)
  }
